...
function FormSection({ selectedTemplate, userFormInput, loading }: props) {
    ...
    return (
        <div className='p-5 shadow-md bg-white rounded-lg border'>
            ...
            <h2 className='font-bold mb-2 text-2xl text-indigo-500'>{selectedTemplate?.name}</h2>
            ...
            <form className='mt-7' onSubmit={onSubmit}>
                {selectedTemplate?.form?.map((item, index) => (
                    <div key={index} className='my-2 flex flex-col gap-2 mb-7'>
                        ...
                        {item.field === 'input' ? (
                            <Input name={item?.name} required={item?.reuired} onChange={handleInputChange} />
                        ) : item.field === 'textarea' ? (
                            <Textarea name={item?.name} required={item?.reuired} onChange={handleInputChange} />
                        ) : null}
